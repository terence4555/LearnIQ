...
export default function ToeflReadingPage() {
  ...
  useEffect(() => {
    fetch("/api/reading?module=toefl")
      .then((res) => res.json())
      .then((data) => {
        setPassage(data);
        setLoading(false);
      ...
      >
        ...
        <SoftCard>
          <button
            ...
            className="rounded-[var(--radius-pill)] bg-[var(--color-secondary)] px-6 py-2.5 font-semibold text-white hover:bg-[var(--color-accent-strong)] transition-colors"
          >
